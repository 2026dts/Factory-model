/*
 ================================================================================
    FACTORY MODEL - COMPLETE SYSTEM (PROFESSIONAL EDITION) - v1.2.1
    ESP32 + ThingsBoard Cloud (MQTT telemetry + two-way RPC + Advanced OTA)

    CHANGELOG vs v1.2.0:
    - Firmware now reports itself: fw_title/fw_version are published as
      CLIENT-SIDE ATTRIBUTES on every successful MQTT connect, and fw_state
      (DOWNLOADING/VERIFIED/UPDATED/FAILED/etc.) is now published as a
      client attribute too, instead of telemetry - matching what ThingsBoard's
      OTA tracking and dashboard widgets actually expect to bind to.
      This is what makes a "current firmware version" widget update itself
      automatically the moment a newly-flashed device reconnects.

    CHANGELOG vs v1.1.0:
    - Added a persisted lifetime product counter (NVS via Preferences), separate
      from the per-boot session counter:
        productCount      -> resets to 0 every reboot (current run)
        productCountTotal -> survives reboots/power loss (lifetime total)
      Writes are batched (every 5 counts OR every 30s, whichever first) to
      limit flash wear, with a forced flush before any reboot/OTA restart.
    - Added a "resetProductCount" RPC method to zero both counters on demand
      (handy before each client demo run).

    CHANGELOG vs v1.0.0:
    - Fixed unsafe serializeJson() calls into fixed char buffers (now bounded)
    - Added OTA "stalled transfer" watchdog (aborts if no bytes for 15s)
    - Added sensor health flags to telemetry (lightingIna_ok, coolingTower1_ok, etc.)
    - Added MQTT Last Will & Testament so ThingsBoard flags the device offline
      immediately on ungraceful disconnect, plus an explicit "online" status push
